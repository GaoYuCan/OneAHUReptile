...
// ALL DECRYPTION FUNCTIONS -----------------------------------------------------------------------------------------
    ...
    public static String encryptAHUPayload(String payload) {
        long key1 = getLongFromBytes(str2Bt("1"), 0);
        long key2 = getLongFromBytes(str2Bt("2"), 0);
        long key3 = getLongFromBytes(str2Bt("3"), 0);
        int r = payload.length() / 4;
        int a = payload.length() % 4;
        byte[] bytes = new byte[r * 8 + (a > 0 ? 8 : 0)];
        for (int i = 0; i < r; i++) {
            long t = encryptBlock(getLongFromBytes(str2Bt(payload.substring(i * 4, (i + 1) * 4)), 0), key1);
            t = encryptBlock(t, key2);
            t = encryptBlock(t, key3);
            getBytesFromLong(bytes, i * 8, t);
        }
        if (a != 0) {
            long t = encryptBlock(getLongFromBytes(str2Bt(payload.substring(r * 4)), 0), key1);
            t = encryptBlock(t, key2);
            t = encryptBlock(t, key3);
            getBytesFromLong(bytes, r * 8, t);
        }
        return hex(bytes);
    }

    private static byte[] str2Bt(String str) {
        byte[] bytes = new byte[8];
        for (int i = 0; i < 4; i++) {
            if (i < str.length()) {
                char v = str.charAt(i);
                bytes[i << 1] = (byte) ((v >> 8) & 0xff);
                bytes[(i << 1) | 1] = (byte) (v & 0xff);
            } else {
                bytes[i << 1] = 0;
                bytes[(i << 1) | 1] = 0;
            }
